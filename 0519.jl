...
using PlutoUI, LinearAlgebra, Distributions,Plots

# ╔═╡ 1ef37d30-6f2b-4ca4-bb5a-86272661ff09
Plots.plotly()
...
begin
    # MvNormal : 다변량 정규분포, 둘 이상의 확률 변수들의 결합 분포
...
# ╔═╡ 898416e4-de5f-4726-9ad7-27ca44147fcd
md"""
### ------------------------------------------------------
"""
...
md"""
## ${\bf Z}={\bf \tilde U}{\bf \tilde D}$의 계산방법
"""
...
# ╔═╡ 05bb7d7f-ee10-4860-8694-39e922a187ce
md"""
### ------------------------------------------------------
"""
...
let 
	Ṽ = V[:,1:2] 
	[Ũ*D̃  X*Ṽ]
	# 둘의 결과가 매우 유사함을 확인할 수 있다. 

	# 이때, Ṽ를 X'X의 SVD 결과인 ΨΛΨ'에서 Ψ̃를 Ṽ로 사용할 수 있다. 
	# 즉, Ψ̃와 Ṽ는 유사함을 알 수 있다. 

	# 아래에서 언급하겠지만,
	# X'X의 SVD말고 고유값 분해를 통해서도 V를 얻을 수 있다. 
	
end 

# ╔═╡ f07d420e-ad67-4d81-867d-56e034421925
V
# 이 matrix의 축소버전을 X에 곱하여 ŨD̃의 대체로 사용할 수 있음
...
md"""
- 결과를 보니까 ${\bf V}$와 ${\bf \Psi}$는 column의 나열 순서만 약간 상이할 뿐 거의 흡사하다고 할 수 있다. 
  - 하지만 이러한 사소한 차이는 무의미하다. 
  - 순서가 다른것은 맞춰주면 되고 부호는 달라져도 상관없다.
"""
...
# 앞 장에서는 U1 = V1 = Ψ
# ΨΛΨ로 표현해줬었다.
# 그리고 이때 Λ=d1은 X'X의 고유값분해에서 고유값들을 대각행렬로 표현해준 것이다..
# 왜 그런지는 나도 몰?루

# ╔═╡ 3e572717-4415-4926-831f-88fb645136d6
[U1 V1]

# ╔═╡ 24bdcfbc-6b83-4bdf-8e6f-4517d2bda6ea
[V U1]

# ╔═╡ b53fe1b2-c12e-4272-ba1f-5ac1f8606984
[V V1]

# ╔═╡ 8120f00a-3949-4e5b-a740-54a469cca04a
md"""
### SUMMARY
- X의 SVD
  - UDV'
  - 이때 V는 Ψ와 유사하며 이는 X'X의 고유벡터이기도 하다.
- X'X의 SVD
  - ΨΛΨ'
  - 이때 Ψ는 고유벡터와 유사
  - Λ는 고유값을 대각행렬로 나타낸 것
...
# ╔═╡ 1935fa68-41ad-4161-b6d2-3b466b9085b9
md"""
### ------------------------------------------------------
"""
...
- 여기서 ${\bf \hat X}$는 X의 축소버전을 의미함
- X와 완전히 같은 것은 아님
- X의 SVD 결과를 그대로 이용한 게 아니라 SVD 결과의 축소버전을 사용했기 때문에
"""
...
let
	λ, Ψ = eigen(X'X)
	# X를 SVD로 분해했을 때 V의 대체로 사용할 수 있었음
	# 위에서 설명했었음
	
	V = [Ψ[:,3] Ψ[:,2] Ψ[:,1]] # 열 배치 순서는 달랐어서 이렇게 수정은 해줘야 함
	Z = X * V[:,1:2] # 얘가 ŨD̃ 역할을 함!
	X̂ = Z * V[:,1:2]'
	[X X̂]
end
...
# ╔═╡ 910635e2-c23e-4f0b-941e-96e8bda84d92
md"""
> 위 두 셀을 결과가 동일할 수밖에 없다.

> 왜? X'X를 SVD수행해준 결과에서 ΨΛΨ로 분해되기 때문에 양쪽 끝 어느 것으로 사용해줘도 결과는 동일하게 된다. 
"""
...
let 
	U1,d1,V1 = svd(X'X) # = ΨΛΨ'
	# X'X의 결과에서 U1 = V1 = V(이 V는 X의 SVD 결과를 의미함)
	
	V2 = [V1[:,1] -V1[:,2] V1[:,3]] # 두번째 컬럼의 부호를 임의로 바꿔버렸음 
	Z = X * V1[:,1:2] # 부호를 안바꾼 V1으로 Z를 구함 (=ŨD̃=XṼ)
	Z2 = X * V2[:,1:2] # 부호를 바꾼 V2으로 Z를 구함 (=ŨD̃=XṼ)
	
	[Z Z2]
	# 당연히 위 둘은 완전히 동일하지 않을 것
	# 하지만 이에 적절한 변형을 해본다면?
end 
...
let 
	U1,d1,V1 = svd(X'X) 
	V2 = [V1[:,1] -V1[:,2] V1[:,3]] # 두번째 컬럼의 부호를 임의로 바꿔버렸음 
	Z = X * V1[:,1:2] # 부호를 안바꾼 V1으로 Z를 구함 
	Z2 = X * V2[:,1:2] # 부호를 바꾼 V2으로 Z를 구함 
	[Z Z2]	# 비교해보면 당연히 Z와 Z2는 달랐었다.
	# 여기까진 위셀과 동일하다.
	...
	X̂2 = Z2 * V2[:,1:2]' 
	[X̂ X̂2]
	# 그런데 reconstruction은 같다.
	# 이때 적절한 변형으로 축소버전을 reconstruction해줄 때도, 부호를 임의조정해주었던 것을 그대로 사용해야 함
end 

# ╔═╡ 87675737-6a0b-4358-bd33-0e3e329da94f
md"""
`-` 결론: 이 예제에서 Z와 Z2는 그 값이 똑같진 않지만 
