...
# ╔═╡ 968702fb-f4d2-4854-858a-e955248f260c
md"""
### 통계학과에서 바라보는 행렬
- 제 생각이에요..
"""

# ╔═╡ 56124b80-ec60-4f07-9b19-476dec970fab
md"""
`-` 경우1: 데이터의 저장수단 
"""

# ╔═╡ 6278889a-b953-4728-8e81-cfe1a3cea112
md"""
`-` 경우2: 이미지
"""

# ╔═╡ 3144f9c4-9c09-4e06-a01b-9cde4dfe95fe
md"""
`-` 경우3: 선형변환 (선형연산)
"""
...
begin
	...
	H = X* inv(X'X) * X' # <- hat matrix, projection matrix, H는 변환을 의미하는 행렬이 된다.
end
...
let 
	scatter(x,y)
	scatter!(H*x,H*y)
	# 변환된 형태를 보면 마치 projection된 것처럼, 사영된 것처럼 변환되었음
end 
...
hani[1,1] # 색이 보이는건 좋은데, 숫자가 보고 싶을때도 있음
...
# ╔═╡ e7ae2970-3807-417d-8f0a-1a2726715760
size(hani)
...
md"""
`-` 그렇다면 역으로  $3\times n \times m$의 텐서를 $n\times m$의 이미지자료를 바꿔서 생각할 수 있도록 하는 함수도 존재할지 궁금하다.
"""
...
typeof(OnePixel), typeof(RGB(1,0,1))
...
# ╔═╡ bf36b329-47fe-4448-9ae6-85e9a639159b
md"""
- 아래 개념하고,
- 위 개념 커널 가우시안 이용해서 스무딩 만드는 거 알아보기
"""
...
# ╔═╡ ef435413-78f0-4907-9ed5-ec9c4136bf8c
let 
	p1 = hani |> (x -> x')
	p2 = hani |> channelview .|>  (x -> x^2) |> colorview(RGB) |> (x->x')
	md"[$p1 $p2]"
end 
