...
function f(z)
	return z+100 
	# 위에서 선언한 z를 여기에서 또 사용할수 있다? 원래는 중복 정의가 안 되는데 ==> 여기의 z는 이 함수에서만 효력이 있음
end

# ╔═╡ 2020a56c-563a-435a-a76d-478ecb16d8ab
f(3)
# 밖에 있는 z는 그대로 0임
# 파이썬에서의 지역변수, 전역변수 개념같음
...
Plots.plotly()
# 앞으로 plot을 할 때 plotly를 backend로 사용할 수 있게 해줌
...
# g(x), 즉 g라는 함수에 x라는 범위를 넣어주면 error가 발생하게 된다.
# 왜냐면 함수 g는 스칼라를 받도록 설계해뒀는데, 벡터를 넣었기 때문에 error가 발생
# 따라서 각각 계산해주어야 함
# 따라서 g((0,1))을 넣어줘도 error가 발생하게 된다.
# 따라서 list comprehension을 이용해야 한다. 	 
# list comprehension을 이용하면 각각 계산하여 list를 채울 수 있게 되는 것이다. 
...
# 따라서 그냥 g(x)를 수행해도 함수값이 벡ㅌ터로 반환되게 된다.
...
# 여기서 말하는 `이런 기능` : 입력이 왔을 때, -1을 하고 제곱을 하는 기능..
...
# ╔═╡ e78eaaf0-40fd-4c0d-8d4d-84e0e1f3b055
# 부가설명 해보자
...
# ╔═╡ 7a8c1b62-fedd-4912-9a00-73138ef1381f
-1,1
...
# ╔═╡ c2af9d85-efb7-4bef-96d6-2d73d47ba441
[0,0.5]
# list안에 원소 두개 넣어준 것

######## 질문
# 그러면 아까 plot(_h(1),-2,2)
# 여기서 -2,2는 -2:2와 같은 역할을 하는 거고
# [-2,2]는 리스트 안에 원소 두개만 넣어준 거?

# 1:3 안에 있는 원소 뭔지 확인하는 거 전 시간에 배운 것 같은데 전 자료 참고해서 정리해두기
...
# ╔═╡ df263896-8a0d-468a-818d-14f09ee00046
1:4
# 이거 안에 있는 원소배열 어떻게 확인하지?
...
# ╔═╡ 8394e540-58c3-4ff6-a4a8-d96379c16389
n
