...
- 마지막의 $g'(\theta)N(0,\sigma^2)$은 평균이 0이고 분산이 $(g'(\theta))^2\sigma^2$인 정규분포를 의미한다. 
"""
...
md"""
> 만약 어떤 변수가 근사적으로 정규분포를 따르면, 그 변수에 연속하는 함수를 씌워도 여전히 정규분포를 따른다. 즉, 평균도 정규분포를 따르고 분산도 정규분포를 따를 수 있다. 
"""
...
- 그런데 $\frac{\bar{X}-\mu}{\sqrt{\sigma^2/n}}=\sqrt{n}(\bar{X}-\mu)\frac{1}{\sigma}$. 여기에서 $\sqrt{n}(\bar{X}-\mu)\frac{1}{\sigma}$ N(0,1)로 수렴하므로 여기에 $\sigma$를 곱한것은 $N(0,\sigma^2)$으로 수렴할 것이다. 즉 
$$\sqrt{n}(\bar{X}-\mu) \overset{d}{\to} N(0,\sigma^2)$$
- 이제 $Y_n=\bar{X}=\frac{1}{n}\sum_{i=1}^{n}X_i$
$$\sqrt{n}(Y_n-\mu) \overset{d}{\to} N(0,\sigma^2)$$
- 따라서 델타메소드의 가정은 "CLT가 성립할때~" 정도의 언어로 바꿔서 기억해도 무방하다.
"""

# ╔═╡ 23974c89-12e5-44ca-95b3-dcd001195270
md"""
`-` 델타메소드의 앞부분은 이해되었음. 델타메소드의 뒷부분을 이해하기 위해서는 테일러 정리를 이해해야 한다. 
"""
...
let 
	f(x)=sin(x)
	plot(f,-π,π)
	f′ = x-> ForwardDiff.derivative(f,x) # 지점x에서의 접선의 기울기를 구하는 함수 f'(x)
	plot!(f′)
	plot!(x->x)
	# 아, 기존에 plot(f,-π,π)로 그려져있으니까 !로 plot 겹쳐그릴 땐 범위 지정하지 않아도 된다.
end 
...
[1,2,3] * 2
# 아 이건 되고 그냥 list 원소끼리 곱셈할 땐 dot필요한가봄 ㅇㅂㅇ
...
begin
	plot(f, -4,4,ylim=(-3,10)) # 아래 셀에서의 lhs, 즉 e^(0.5x)
	plot!(f̃) # 아래 셀에서의 rhs
	scatter!([a],[f(a)])
end
...
`-` Delta method: $\sqrt{n}(g(\bar{X}_n)-g(p) ) \to g'(p) N(0,p(1-p))$

`-` Plug in $g(x) = x(1-x)$ and $g'(x)=1-2x$.

$$\sqrt{n}(\bar{X}_n(1-\bar{X}_n)-p(1-p) ) \to  N\big\{0,p(1-p)(1-2p)^2 \big\}$$

"""

# ╔═╡ fd8062e9-3dcb-484e-aca1-338ef4c18e0a
md"""
> 평균의 분포는 CLT에 의해서 잡혔고, 분산의 분포는 delta method에 의해 잡힘..
"""
...
md"""
-  p =  $\frac{1}{2}$일때는 $\bar{X}_n(1-\bar{X}_n)$의 점근적분포에서 분산이 정의되지 않는군!!
...
- 근사: y축은 실제로는 곡선에 의한 상이지만 근사적으로 직선에 의한 상이라고 생각해도 무방. 
