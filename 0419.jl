...
30개의 샘플을 확보하여 $\xi=\frac{1}{30}\sum_{i=1}^{30}(x_i-7)^2$`(편차제곱의 평균이므로 분산 구하는 식)`를 계산하였으며 계산결과 $\xi=2.72$가 나왔다고 하자. 
...
- 관찰: 우리예제의 경우에는 $H_0$가 참이라는 가정(분산이 4인..)하에 $\sum_{i=1}^{30}(\frac{X_i-7}{2})^2\sim \chi^2(30)$
...
# ╔═╡ c9fd0deb-f7f3-4fc8-b566-46fcd2662306
quantile(ξs,0.05)
# 이것과 값이 매우 유사
...
- 결: $H_0$가 참일지 $H_1$이 참일지 판단. 절대적인 기준은 없음. (하지만 굉장히 보수적인(H₀를 지지하는) 사람이라도 $p$-value가 0.05보다 작으면 $H_1$이 참이라고 인정)
"""
...
- X의범위: $x\in (0,\infty)$
- parameter의 의미: $k$는 자유도, 표준정규분포 제곱을 몇개 합쳤는지.. 
- parameter의 범위: $k=1,2,3,4,\dots$ 
- E(X): $k$
- V(X): $2k$
"""
...
# rand(Normal(0,1),4).^2 |> sum 이게 카이제곱분포 1개임
# 카이제곱분포 100개 만들기
# Normal(0,1)  => 표준정규분포

# ╔═╡ 7e6677f1-1df0-4518-a78d-7619486d68b9
md"""
(방법3) 지수분포 -> 카이제곱분포
- $X,Y \overset{iid}{\sim} N(0,1) \Rightarrow R^2/2 \sim Exp(1)$
- $where R^2=X^2+Y^2$
- $X = Rcosθ, Y = Rsinθ$
"""
...
# rand(Exponential(1)) *2 는 R²
# R²은 X²+Y²
# 따라서 R²+R²은 X²+Y²+X'²+Y'²
# 즉 표준정규분포 네개의 제곱합으로서 
# 자유도가 4인 카이제곱분포를 의미한다. 

# ╔═╡ a6c65dc1-e84c-4d35-a8f5-2d0c9db6f045
[rand(Exponential(2)) + rand(Exponential(2)) for i in 1:100]
# 자유도 4인 카이제곱분포
# Exponential(2)가 X² + Y²임
# 따라서 표준정규분포 4개 구하려면 2Exponential(2)를 하면 안됨
# 2Exponential(2)이건 평균이 4인 지수분포로서 표준정규분포제곱합과 관련없음
# 표준정규분포 제곱합과 관련있는 건 평균이 2인 지수분포임
# Exponential(2) + Exponential(2) 이렇게 두개를 더해줘야 함

# ╔═╡ f9be81aa-ccf2-4704-9273-0e25e6ce1b01
md"""
### 자유도4 카이분포만드는 방법들
"""
...
# 자유도가 2인 경우는 그냥 지수분포....
# 당연하지
# 자유도가 2인 카이제곱분포는 표준정규분포 제곱 두개를 합친 것이고, 표준정규분포 제곱 두개를 합친 것은 평균이 2인 지수분포를 따르기 때문! 

# 자유도가 커짐에따라 지수분포모양에서 정규분포모양으로 가까워짐....
...
md"""
### 감마분포 요약
- X의 의미: **서로 독립인 지수분포**를 $\alpha$개 합친 것, 시간1에 평균적으로 $\lambda$번 발생하는 사건이 있을때 $\alpha$번째 사건이 발생할때까지 걸리는 시간.
- X의범위: $x \in (0,\infty)$
  - 당연히 지수분포가 양수이니 합쳐도 양수일 것.
- parameter의 의미: $\alpha=$ 지수분포를 더한 횟수(의 확장버전, 1.5개 이런 것도 가능), $\beta=\frac{1}{\lambda}=$ 지수분포의 평균
- 파라메터의 범위: $\alpha>0$, $\beta>0$.
  - 만약 (3,2)였으면, 평균이 2인 지수분포를 3개 합친 것을 의미한다. 
...
# ╔═╡ 350fecab-84a4-4527-9c85-a02f6520c895
let
	p = rand(Gamma(1.5,2),100000) |> histogram
	q = rand(Chisq(3),100000) |> histogram
	plot(p,q,layout=(1,2))
end
...
# rand(Exponential(β),α) |> sum 이게 감마분포 하나 생성한 것임
...
# rand(Normal(0,1),2).^2 |> sum 는 평균이 2인 지수분포 하나
# 표준정규분포제곱 두개 합

# rand(Normal(0,1),4).^2 |> sum 는 평균이 2인 지수분포 두개의 합
# 표준정규분포제곱 네개 합

# 따라서 해당 셀은 평균이 2인 지수분포 α개의 합
# 표준정규분포제곱 2α개 합
...
let
	...
	# 모든 베타값에 의해서 네개의 히스토그램이 모두 동일한 형상을 띄는 게 아님
	# 여기서 베타값은 지수분포의 평균을 의미한다. 
	# 왜??
	# Exponential(2)가 표준정규분포 제곱 두개 합임
	# Exponential(3),Exponential(4),Exponential(5) 이런 건 표준정규분포제곱합과 관련 X
	# 따라서 카이제곱과도 관련 없어짐 
	# 그런데 X1과 X2는 β값과 α값에따라 동일한 분포를 띄게 됨
	

	# 음 정리하자면 표준정규분포 제곱합과 지수분포 그리고 카이제곱분포 감마분포는 관련이 있는데 표준정규분포 제곱합을 짝수개로 더하면 카이제곱분포와 연결시킬 수 있고, 물론 표준정규분포를 짝수개로 더해야만 카이제곱분포로 연결시킬 수 있는 건 아니다. 아무튼 이는 평균이 2인 지수분포 여러개를 더한 것으로도 연결시킬 수 있다. 
...
- 감마분포의 β값이 2 $\to$ 즉, 평균이 2인 지수분포를 k/2개 더했다.
- 평균이 2인 지수분포를 이분의 케이개만큼 더했다는 건
- 표준정규분포제곱합 2개 묶음을 이분의 케이캐만큼 더했다는 것과 일맥상통한다. 
- 지수분포 평균이 2 $\to$ 카이제곱 분포와도 관련!
  - 평균이 2인 지수분포 하나는 표준정규분포 제곱 2개 합과도 동일, 그것은 카이제곱분포와도 연결
...
let
	...
	# 자유도가 3인 카이제곱분포
	# 즉 표준정규분포 제곱합이 3개 더해진 것
	# 즉, 평균이 2인 지수분포가 1.5개 더해져야 함
	# 이걸 감마분포를 이용하여 표현한 게 아래 줄임
...
md"""
α = $(@bind shape Slider(0.1:0.1:30, show_value=true))
β = $(@bind scale Slider(0.1:0.1:100, show_value=true))
"""

# ╔═╡ a33ff421-8344-44ca-8445-e625b73a97a0
md"""
### α값과 β값에 따라 축 간격이나 그래프 모양이 어떻게 변하는지 체크해보기
"""
...
begin
	...
	histogram!(rand(Gamma(6,6),10000)+rand(Gamma(19,6),10000))
end

# ╔═╡ c670a01d-bd87-49f5-8db4-a34395cfad3b
# let
# 	histogram(rand(Gamma(5,2),10000))
# 	a = Array(0.1:0.2:1)
# 	x = Array(1.0:2 *500)
...
# 	histogram(rand(Gamma(5,2),100000))
# 	histogram!(x)
...
- 경우1: $X\sim Exp(\theta)$, $f(x)=\frac{1}{\theta}e^{-\frac{x}{\theta}}$.
  - 평균은 θ
- 경우2: $X\sim Exp(1/\lambda)$, $f(x)=\lambda e^{-\lambda x}$. (최규빈 교수님 채택)
  - 평균은 평균은 $\frac{1}{\lambda}$
...
# ╔═╡ e906e6b4-9c30-4765-9874-7b2229364d4a
let
	x1 = [rand(Exponential(2)) + rand(Exponential(2)) + rand(Exponential(2))+ rand(Exponential(2))+ rand(Exponential(2))+ rand(Exponential(2))+ rand(Exponential(2))+ rand(Exponential(2))+ rand(Exponential(2))+ rand(Exponential(2)) for i in 1:10000] |> histogram
	x2 = rand(Chisq(20),10000) |> histogram
	x3 = [rand(Exponential(2),10) |> sum for i in 1:10000] |> histogram
	plot(x1,x2,x3,layout=(1,3))
end
