...
md"""
`-` 위치모수, 척도모수 
- 1) 확률변수 $Z$가 분포A를 따를때 $Z+b$도 분포A를 따름 $\Rightarrow$ 분포A는 **위치모수**를 가짐 
- 2) 확률변수 $Z$가 분포B를 따를때 $aZ$도 분포B를 따름 $\Rightarrow$ 분포A는 **척도모수**를 가짐 
- 3) 확률변수 $Z$가 분포C를 따를때 $aZ+b$도 분포C를 따름 $\Rightarrow$ 분포A는 **위치모수**와 **척도모수**를 가짐
"""
...
- 분포 C - 분포 B
  - {지수분포, 감마분포}
  - ? 분포B랑 분포C랑 자리 바꿔야하는 거 아닌가?...
"""

# ╔═╡ ee152cba-b44f-4214-878c-448881719f36
md"""
##### **특정분포가 위치모수를 가지는지 척도모수를 가지는지 어떻게 따질까?**
- 이론: $Z$의 pdf가 $f_Z(x)$일 때, $aZ+b$의 pdf는 $f_{aZ+b}(x)=\frac{1}{a}f_Z(\frac{x-b}{a})$
...
# ╔═╡ 63b91904-c515-4d61-b25e-b29cf7eba643
let
	# 평균이 1/5인 지수분포
	p1 = rand(Exponential(1/5),1000) |> histogram
	# 평균이 5인 지수분포
	p2 = rand(Exponential(5),1000) |> histogram
...
이다. 그런데 이것은 **평균이 6인 지수분포의 pdf**와 모양이 같다. 일반화하면 

$Z \sim Exp(1/\lambda) \Rightarrow aZ \sim Exp(a/\lambda)$

라고 볼 수 있다. 

- 즉 지수분포를 따르는 확률변수에 임의의 상수값을 곱해도 여전히 지수분포를 따르므로 지수분포는 척도모수를 가진다고 볼 수 있다. 
> **확률변수 Z가 평균이 2인 지수분포를 따른다면, 확률변수 $3Z$는 평균이 6인 지수분포를 따른다** 
"""
...
$f_{Z+3}(x)=f_Z(x-3)=\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{1}{2}(\frac{x-3}{\sigma})^2}$

이다. 그런데 이것은 평균이 3이고 분산이 σ²인 정규분포의 pdf와 모양이 같다. 일반화하면 

$Z \sim N(0,\sigma^2) \Rightarrow Z+b \sim N(b,\sigma^2)$

라고 볼 수 있다. 

즉, 정규분포를 따르는 확률변수에 임의의 상수값을 더해도 여전히 정규분포를 따르므로 정규분포는 위치모수를 가진다고 볼 수 있다. 
"""
...
- 이건 평균이 몇인 지수분포인데?
  - 답할 수 없다
  - 즉, 이것은 지수분포의 pdf중 한 형태라고 볼 수 없다. 
  - 즉, 지수분포에서 어떠한 상수를 더한것이 지수분포라고 볼 수 없다. 
  - 즉, 지수분포는 위치모수를 가지지 않는다. 
"""
...
let 
	...
	# Normal에서 평균 다음 자리는 분산 자리가 아니라 표준편차 자리임
...
# ╔═╡ ed20bd84-98d9-4996-b18c-9e78ca8f1724
let 
	N = 10000
	
	rand(Exponential(2),N) |> histogram
	# 평균이 2인 지수분포는 자유도 2인 카이제곱분포를 따름. 즉 표준정규분포제곱 2개의 합과 동일
	rand(Chisq(2),N) |> histogram!
...
$(rand(Exponential(2),100000) .* 2 |> histogram)
"""
