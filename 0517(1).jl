...
- 여기에서 $n>p$ 를 가정한다.
- 일반적으로 observation이 feature 수보다 많기 때문에
"""

# ╔═╡ 762c7cf9-73ab-4fce-9f74-177140560eec
md"""
- 소망
  - (1) ${\bf X}$가 가지고 있는 정보는 거의 유지하면서 
  - (2) 저장비용은 좀 더 저렴한 매트릭스 ${\bf Z}$를 만들고 싶다.
"""

# ╔═╡ 989e6e04-1d01-41be-9d77-973089e0264b
md"""
소망 (2)의 의미 :  ${\bf Z}$의 차원이 $n\times q$ 이며 $q<p$. 

소망 (1)의 의미 :  ${\bf Z}$에서 적당한 변환 ${\bf B}$를 하면 언제든지 ${\bf X}$와 비슷한 매트릭스로 복원(reconstruction) 할 수 있다. 즉

${\bf Z}{\bf B} \approx {\bf X}$

을 만족하는 적당한 ${\bf B}$가 존재한다. (이렇게 되면 변환 ${\bf B}$의 차원은 $q \times p$가 되어야 한다.)
...
md"""
`-` 그런데 적당한 $q<p$ 에 대하여 p까지 가지 않고 q까지 더해주는 다음의 식이 만족한다고 가정하자
...
- p까지 안 가고 q에서 끊었기 때문에 ${\bf \tilde U}{\bf \tilde D}{\bf \tilde V}^\top$
"""
...
md"""
`-` ${\bf X}$ 설정 (이 매트릭스를 차원축소하고 싶다. 거의 손실없이. 차원축소해서 150x2로 만들고 싶다)
"""
...
이때 UD와 Vᵀ를 차원축소 시켜 tilde로 표현한다.
"""
...
# ╔═╡ b1af7322-3504-45ae-9196-5c85aea40dcd
md"""
- 왜 이 방법이 고수?
  - 이유 : $${\bf X}^\top {\bf X}$$를 ${\bf SVD}$ 하는 게, 더 큰 차원인 ${\bf X}$가 있다면 ${\bf SVD}$로 분해하는 것보다 더 실용적으로 분해할 수 있음  
"""
...
- 이유 ?  ${\bf X}^\top{\bf X}$를 SVD로 분해하면 ${\bf U}{\bf D}{\bf V}^\top$로 가능한데 가운데 ${\bf D}$는 ${\bf \Lambda}$로 명칭을 바꿨다고 생각하고 그 양 옆의 ${\bf U}$와 ${\bf V}$는 동일한 ${\bf \Psi}$로 대체할 수 있기 때문에 ${\bf X}^\top{\bf X} ={\bf \Psi} {\bf \Lambda} {\bf \Psi}^\top$로 쓸 수 있게 된다. 
"""

# ╔═╡ 3a016304-9990-4f1a-8974-692f49058376
Ψ,Λ,Ψ= svd(X'X)
# 0519 파일에서 설명하겠지만
# X'X의 SVD결과에서 Ψ는 X의 SVD 결과에서 V와 매우 유사함을 알 수 있다. 

# ╔═╡ 71aaa66c-4ca3-4aa2-b0e6-88dc0e9db611
Ψ,V
# 유사함을 알 수 있다. 

# ╔═╡ ab6eabbf-e5ef-4632-a524-c3eed9dc3605
begin
	a=svd(X'X)
	a.Vt
end

# ╔═╡ 5179b260-6fed-4fad-b0df-e0c31c2c668b
md"""
### 질문
- 그럼 $\Lambda$는 어디에 사용?
"""
...
- Ψ는 총 네개의 칼럼으로 이루어져 있는데 그중 첫 번째 칼럼과 두 번째 칼럼만!
"""
...
# ╔═╡ 4be3a1aa-3f74-4f9d-b170-1fbe41bbf71b
md"""
> ŨD̃(하수의 방법) = XṼ(고수의 방법)

> 이때 Ṽ는 X'X의 SVD결과인 ΨΛΨ'에서 Ψ랑 V랑 비슷해서 Ψ̃를 Ṽ로 사용해주었음 
"""
