...
md"""
`-` 아래의 포인트를 파악하는 것이 중요. 
- 포인트1: ${\bf X}$의 svd 결과인 v-matrix (right singular matrix) 는 ${\bf X}^\top {\bf X}$의 고유벡터 행렬에서 구할 수 있고 ${\bf X}$의 u-matrix (left singular matrix) 는 ${\bf X}{\bf X}^\top$의 고유벡터 행렬에서 구할 수 있다. // (이때 ${\bf X}^\top {\bf X}$ 혹은 ${\bf X}{\bf X}^\top$ 는 실대칭행렬이므로 모든 column들이 서로 직교하고 full rank인 고유벡터행렬을 가질 수 있다, recall the spectral thm.)
