...
let 
	...
	- 아니야, 이건 사실.... (무한반복)
	- 느낌: 즉, 매우 작은 시간에 엄청 작은 확률의 베르누이 시행이 독립적으로 엄청 많이 반복되는 느낌을 기억하세요!
	$(_p)
	- 위: 유니폼 -> 베르누이 -> 이항분포 -> 포아송 
	- 아래: 포아송
	"""
...
let 
	...
	p1 = X.+Y |> histogram # 포아송의 합은 다시 포아송이 된다.
...
let 
	...
	p3=[(mean(rand(Poisson(λ),n))+var(rand(Poisson(λ),n)))/2  for k in 1:100] |> histogram # 평균과 분산을 평균 낸 것
	# 그래도 평균추정이 제일 정확해보인다.
	# 섞어서 p3 만들어봤자 p2가 정확도가 떨어져서 p3는 좋지 않다.
...
- 그냥 mean(X)가 더 추정을 잘 하는 것 같으니 ..=평균추정량=분산추정량 이라고 주장하면 안되나? => 됩니다! 이게 바로 MLE(최대우도추정량)에요! 
"""
...
- X의범위: 시간은 양수이므로 X ≥ 0
- 파라메터의 의미와 범위: (1) λ = 시간1에 평균적으로 발생하는 이벤트의 수 (2) 1/λ = 한번의 이벤트가 발생할때까지 평균적으로 걸리는 시간; 이때, λ>0 
...
- 따라서 언뜻생각하면 포아송과 지수분포는 역의 관계라서 포아송분포를 만들고 역수를 취하면 지수분포를 쉽게 만들 수 있을 것 같다.(사실은 안 됨)
"""
