...
- X의범위: X=0 or X=1 
- parameter의 의미와 범위: p는 성공확률을 나타냄, p ∈ [0,1]
  - parameter = 모든 분포에서 괄호 안에 있는? 거라고 생각하면 된다.
- pdf: 수리통계학 책 참고
- mgf: 수리통계학 책 참고
- E(X): p
- V(X): p(1-p)
...
md"""
(방법1)
- julia에서 제공하는 package 사용
"""

# ╔═╡ 8d0273f1-a164-484d-967a-8737d0382435
# using Distributions
# 첫 셀에서 이미 수행해서 error 뜨긴 하지만
# 수행해주어야 함
...
# ╔═╡ ba9b7321-8c03-4a46-bac0-1a0cc97efc75
mean(rand(Bernoulli(0.37),100))
...
rand(100) # 유니폼(0~1사이)에서 100개의 샘플 추출

# ╔═╡ 435bbf1e-531b-4a0e-9b69-3b756e2371b3
rand(100) .< 0.37 # 0.37보다 작은것만 성공

# ╔═╡ a82922eb-753d-4771-a033-6873e7473abe
# rand(100) < 0.37   ERROR 발생
# rand(100)은 100개니까 broadcasting 필요!
...
# ╔═╡ 1d7fb00d-9aff-480f-a00f-5737799e405c
md"""
- n이 커질수록 두 분포가 잘 겹침을 알 수 있다.
"""

# ╔═╡ e9225084-83a7-4730-838c-9a99ecf3e16b
md"""
let ~ end
...
let 
	...
	`-` 평균과 분산의 추정 
	- mean: $p
	- mean est: $(mean(X)) # 평균 추정
	- variance: $(p*(1-p))
	- variance est: $(var(X)) # 분산 추정
	"""
end 
...
# ╔═╡ d860e4fd-a4c1-4e15-bf0a-a6110b6485ea
md"""
- 분산이 가장 클 땐, p=0.5일 때.
  - 예를 들어 p = 1이라고 하면 x는 항상 1만 나오고, p = 0이라고 하면 x는 항상 0만 나오고
  - 분산이 없는 것이다. 
  - 똑같은 값만 나오니까.
  - p가 0.5일 땐, x가 0도 나오고 1도 나오고 골고루 퍼져서 나올 수 있게 된다.
  - 그래서 p가 0.5일 때 분산이 가장 크다
"""
...
# ╔═╡ d1238c38-87bf-414d-a4ee-bbb21c05a52d
md"""
--------
"""
...
- X의의미: 성공확률이 p인 n번의 시행에서 성공한 횟수를 X라고 한다. 
  - 한 번의 시행이 아닌 n번의 시행이다. 
  - 베르누이분포는 한 번의 시행이었음
- X의범위: X=0,1,...,n
- parameter의 의미와 범위: n은 시행횟수, p는 성공할 확률; n=1,2,3,4,... p ∈ [0,1]
- pdf:
- ***mgf: (베르누이분포의mgf)ⁿ
  - 왜?
  - 베르누이분포의 합이 이항분포가 되기 때문에!
- E(X): np
- V(X): np(1-p)
"""

# ╔═╡ 9773bd63-0554-4c21-813d-32c568e6d6e8
md"""
-------
...
- 이때, Z는 각각 서로 독립인 베르누이 분포라는 것이다.
- d : 분포적으로 따른다.
- 베르누이 sum = 이항분포

"""

# ╔═╡ 33c3dbda-f0d4-4596-87f5-46db07ffc118
md"""
---
"""
...
# 원소 8개인 100개의 list에 대해서 각각 sum
# 즉 sum rkqtdl 100개 나올 것이다. 
...
let 
	...
	`-` 평균과 분산의 추정 
	- mean: $(n*p) # 이항분포의 평균
	- mean est: $(mean(X)) # 이항분포 평균 추정
	- var: $(n*p*(1-p))# 이상분포의 분산
	- var est: $(var(X)) # 이항분포 분산 추정
	"""
end
...
# ╔═╡ 4fb46196-f582-4fff-8237-0017d4c5b3cf
md"""
- 0.5일 때, 분산이 가장 커짐으로써 골고루 퍼져있음을 알 수 있다.
"""
...
# ╔═╡ ecac0ca2-4352-4bc4-94a6-f6d95a857b37
let 
	n=30
	p=0.45
	method1 = rand(Binomial(n,p),100) 
	method2 = [rand(Bernoulli(p),n) for i in 1:100] .|> sum 
	method3 = [rand(n) .< p for i in 1:100] .|> sum
	histogram([method1,method2,method3],alpha=0.5)
end 
# bin 설정 필요 X
